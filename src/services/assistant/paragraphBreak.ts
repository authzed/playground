 ...
 * last one.
 *
 * Mirrored in api/_lib/paragraphBreak.ts for the server-side turn loop —
 * Vercel's per-function bundler only resolves imports within api/, so this
 * can't be a single shared module (see the same constraint noted in
 * api/share.ts). Keep the two in sync.
 */
