...
function applyChangesInPlace<N extends VisNode, E extends VisEdge>(
  visNodeSet: VisDataSet,
  visEdgeSet: VisDataSet,
  existingGraph: VisGraphDefinition<N, E>,
  ...
  if (
    existingGraph.edges.length !== graph.edges.length ||
  ...
  const graphUpdateDebouncer = useDebouncer(
    (graph: VisGraphDefinition<N, E>) => {
      ...
      const network = new vis.Network(
        divRef.current,
        { nodes: nodeDataSetRef.current, edges: edgeDataSetRef.current },
        {
          nodes: {
            shape: "dot",
            size: 10,
