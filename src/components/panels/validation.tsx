...
export function ValidationSummary(props: PanelSummaryProps) {
  return (
    <TabLabel
      icon={
        <PlaylistAddCheckIcon htmlColor={props.services.validationService.validationStatusColor} />
      }
...
export function ValidationPanel(props: PanelProps) {
  ...
  return (
    ...
    >
      {validationState.status === ValidationStatus.NOT_RUN && <span>Validation Not Run</span>}
      {validationState.status === ValidationStatus.CALL_ERROR && (
        <span>Validation Call Failed. Please try again shortly.</span>
      )}
      {validationState.status === ValidationStatus.RUNNING && <CircularProgress />}
      ...
      {validationState.status === ValidationStatus.VALIDATION_ERROR && (
        <span>
          {validationState.validationErrors?.map((de: DeveloperError, index: number) => {
            return (
